...
using MvcSignalRChatWithWebAPI.Models;
        ...
        public async Task<ActionResult> Manage(ManageUserViewModel model)
        {
            ...
            {               
                if (ModelState.IsValid)
                {
                    ...
                    if (result.Success)
                    {
                        return RedirectToAction("Manage", new { Message = "암호가 변경되었습니다." });
                    }
                    else
                    {
                        AddErrors(result);
            ...
            {
                // 사용자에게 로컬 암호가 없으므로 누락된 OldPassword 필드로 인해 생성된 모든 유효성 검사 오류를 제거하십시오.
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (result.Success)
                    {
                        return RedirectToAction("Manage", new { Message = "암호가 설정되었습니다." });
                    }
                    else
                    {
                        AddErrors(result);
